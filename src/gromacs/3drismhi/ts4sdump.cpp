                            ...
                            if (work==WORK_CHECK_DATA){
                                ...
                                if (crc32_this_frame != header.crc32){
                                    ...
                                    if (uret>0){
                                        ...
                                        if (uret==0){         fprintf(stderr, "%s : %d at #%d : unknown error\n", software_name, uret, nframe);
                                        } else if (uret==-1){ fprintf(stderr, "%s : %d at #%d : memory overflow\n", software_name, uret, nframe);
                                        } else if (uret==-2){ fprintf(stderr, "%s : %d at #%d : unknown precision\n", software_name, uret, nframe);
                                        } else if (uret==-3){ fprintf(stderr, "%s : %d at #%d : compressor unmatched\n", software_name, uret, nframe);
                                        } else if (uret==-4){ fprintf(stderr, "%s : %d at #%d : uncompressor version too low\n", software_name, uret, nframe);
                                        } else if (uret==-5){ fprintf(stderr, "%s : %d at #%d : unknown compressor\n", software_name, uret, nframe);
                                        }
                                    }; //printf("%d\n", uret);
